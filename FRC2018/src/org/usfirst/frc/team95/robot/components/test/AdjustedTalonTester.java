...
public class AdjustedTalonTester extends JFrame implements TestCanSpeedController.Listener {
	...
	public void runThrottleTest() {
		...
		for(double voltage = 10; voltage >= 5.0; voltage -= 0.5) {
			...
			XYSeries series = new XYSeries("Adjusted throttle vs input at " + pdp.getVoltage() + "V", false);
	...
	public void runVoltageTest() {
		...
		for(double throttle = -1.0; throttle <= 1.0; throttle += 0.2) {
			...
			XYSeries series = new XYSeries("Adjusted throttle vs voltage at throttle " + throttle, false);
		...
		// Create chart
		JFreeChart chart = ChartFactory.createScatterPlot("Adjusted throttle vs voltage", "voltage", "Adjusted throttle",
				dataset);
